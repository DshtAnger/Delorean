...
#### 工作经验
* 2017年8月 -- 至今 | 华为@内部网络安全实验室-泛网络渗透组 | 二进制漏洞挖掘工程师  
...
* 2020年04月15日 -- 2020年04月30日 | 数据通信大型基础设施蓝军保障测试
	...
	* 项目成果：两周时间对设备的管理面协议NETCONF、RADIUS、SNMPv3和控制面DHCP、PPP协议做Peach Fuzz为主，辅以其他面自由发散测试。打通协议配置后Peach对5个协议均迭代超过8天未现异常，协议健壮性可靠。自由测试发现构造某特定RESTCONF请求可导致设备拒绝服务后重启，连接型资源耗尽可导致NETCONF拒绝服务。长亭科技未发现有效问题。
...
* 2018年07月15日 -- 2018年08月05日 | USG6600防火墙测试
	...
	* 项目成果：临时抽检项目，以用户面协议的Peach Fuzz为主，测试RIPv2认证模式和OSPF邻居发现消息；AFL Fuzz开源组件json-C 0.12.1的反序列化接口；测试Web管理后台。协议Fuzz均无异常，Web管理后台处发现仅具有监控权限的低权限Web用户可直接通过调用/cmd.html接口执行设备CLI命令。
...
* 2018年01月10日 -- 2018年02月10日 | 云化统一数据节点CloudUDN保障测试
	...
	* 项目成果：发现vOSMU侧提供节点扩容缩容的业务存在4处命令注入可拿ossadm权限，后台服务器上发现2处Sudo提权脚本，结合可从外部无权限获取vOSMU节点Root权限。vLDA侧发现消息压缩处理中1处越界写非法内存，SSL连接信息管理存在内存堆积导致进程崩溃，某模块Notify消息死循环，某消息状态机切换导致死循环。
...
* 2014年10月 -- 2016年10月 | 西安尚赛技术有限公司 | CCIE R&S | 校外培训机构
	* 学习思科认证网络工程专家CCIE路由交换方向，通过了CCNA、CCNP认证实操考试，熟悉多种常用IGP/EGP类路由协议原理和配置细节，具备实际企业园区网组网、排错、调优和安全部署等能力。
